...
int main() {
    ...
    rescaleRT.create(img_dim, img_dim);
    ...
    io_descs[0] = IO_Desc(Int2(img_dim, img_dim), IO_Type::prediction, 32, 1.0f);

    Hierarchy1 h;
    h.init_random(io_descs, lds);

    Array<Vec1> input_vecs(img_dim * img_dim);
    ...
    do {
        ...
        for (int x = 0; x < rescaleRT.getSize().x; x++)
            for (int y = 0; y < rescaleRT.getSize().y; y++) {
                ...
                if (mono > 0.5f)
                    input_vecs[y + x * rescaleRT.getSize().y] = vecs[1];
                else
                    input_vecs[y + x * rescaleRT.getSize().y] = vecs[0];
            }

        // Feed first 5 frames from image, even when generating ("seed" sequence)
        if (simFrame > 5 && genMode) {
            auto pred = h.get_prediction_vecs(0);

            for (int i = 0; i < input_vecs.size(); i++)
                input_vecs[i] = (pred[i].dot(vecs[0]) > pred[i].dot(vecs[1]) ? vecs[0] : vecs[1]);
        ...
        for (int x = 0; x < rescaleRT.getSize().x; x++)
            for (int y = 0; y < rescaleRT.getSize().y; y++) {
                ...
                Vec1 v = pred[y + x * rescaleRT.getSize().y];

                c.r = c.g = c.b = (v.dot(vecs[0]) > v.dot(vecs[1]) ? 0 : 255);;
