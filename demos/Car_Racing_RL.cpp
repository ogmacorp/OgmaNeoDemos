...
std::string convert(std::string s, int numRows) {
    ...
    int numPerZag = (numRows - 1) * 2;
    int zagsPerS = s.length() / numPerZag + 1;
...
int main() {
    ...
    ioDescs[1] = Hierarchy::IODesc(Int3(1, 1, steerResolution), IOType::action);
    ...
    do {
        ...
        float reward = 0.1f * std::abs(car.speed) * (carDir.x * trackDir.x + carDir.y * trackDir.y) + (reset ? -10.0f : 0.0f);
