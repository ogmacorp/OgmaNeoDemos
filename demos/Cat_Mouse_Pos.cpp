...
public:
    ...
    void update(
        const Int_Buffer &pred_cis
    );

    const Int_Buffer &get_cis() const {
        return cis;
    }

    const Float_Buffer &get_memory(
    ) const {
        return memory;
    }
};

void Positional_Memory::init(
    int num_cells,
    int res
) {
    this->res = res;

    memory = Float_Buffer(num_cells, 0.5f);

    cis = Int_Buffer(num_cells, 0);
}

void Positional_Memory::update(
    const Int_Buffer &pred_cis
) {
    // decode
    for (int i = 0; i < memory.size(); i++) {
        float decoded = pred_cis[i] / static_cast<float>(res - 1);

        memory[i] = std::fmod(memory[i] + rate * (decoded * 2.0f - 1.0f), 1.0f);

        if (memory[i] < 0.0f)
            memory[i] += 1.0f;

        // encode again
        cis[i] = static_cast<int>(((memory[i] - decoded) * 0.5f + 0.5f) * (res - 1) + 0.5f);
    }
}
...
int main() {
    ...
    Positional_Memory memCat;
    memCat.init(4, 16);

    Positional_Memory memMouse;
    memMouse.init(4, 16);

    Array<Hierarchy::IO_Desc> ioDescs(3);
    ioDescs[0] = Hierarchy::IO_Desc(Int3(7, 5, obsRes), IO_Type::prediction, 4, 8, 2, 2); // observation
    ioDescs[1] = Hierarchy::IO_Desc(Int3(1, 3, actionRes), IO_Type::action, 4, 8, 1, 2); // action
    ioDescs[2] = Hierarchy::IO_Desc(Int3(2, 2, 16), IO_Type::prediction, 4, 8, 1, 2); // positional memory
    ...
    Array<Int_Buffer_View> catInputs(3);
    catInputs[0] = catObsi;
    catInputs[1] = catActionsi;
    catInputs[2] = memCat.get_cis();

    Array<Int_Buffer_View> mouseInputs(3);
    mouseInputs[0] = mouseObsi;
    mouseInputs[1] = mouseActionsi;
    mouseInputs[2] = memMouse.get_cis();
    ...
    do {
        ...
        for (int ss = 0; ss < numSubSteps; ss++) {
            ...
            if (aiTimer >= aiDT) {
                ...
                memCat.update(hCat.get_prediction_cis(2));
                memMouse.update(hMouse.get_prediction_cis(2));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            rs.setSize(sf::Vector2f(32, 128));
            rs.setFillColor(sf::Color::Transparent);
            rs.setPosition(sf::Vector2f(i * 48 + 20, 20));
            ...
            rs.setSize(sf::Vector2f(32, 128 * memCat.get_memory()[i]));
