...
int main() {
    ...
    std::vector<sf::Color> pallette2(e.numColumns);
    ...
    do {
        ...
        for (int it = 0; it < iters; it++) {
            ...
            std::vector<float> inputs = { point.x, point.y };
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            cs.setPosition(data[i].x, -data[i].y);

            window.draw(cs);
        }

        // Draw SOMs as lines
        sf::VertexArray va(sf::LineStrip, e.numCells);
        
        for (int c = 0; c < e.numColumns; c++) {
            for (int cc = 0; cc < e.numCells; cc++) {
                if (e.hiddenRates[cc + e.numCells * c] == 1.0f)
                    continue;

                va[cc].position.x = e.protos[0 + 2 * (cc + e.numCells * c)];
                va[cc].position.y = -e.protos[1 + 2 * (cc + e.numCells * c)];

                va[cc].color = sf::Color::White;
            }
