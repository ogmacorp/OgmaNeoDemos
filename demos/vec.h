...
public:
    ...
    Vec<S, L> operator*(
        ...
        for (int i = 0; i < S; i++) {
            ...
            result[i] -= (result[i] >= L) * L; // modulo but faster
        }
    ...
    const Vec<S, L> &operator*=(
        ...
        for (int i = 0; i < S; i++) {
            ...
            buffer[i] -= (buffer[i] >= L) * L; // modulo but faster
        }
    ...
    Vec<S, L> operator/(
        ...
        for (int i = 0; i < S; i++) {
            result[i] = buffer[i] + L - other.buffer[i];
            result[i] -= (result[i] >= L) * L; // modulo but faster
        }
    ...
    const Vec<S, L> &operator/=(
        ...
        for (int i = 0; i < S; i++) {
            buffer[i] = buffer[i] + L - other.buffer[i]; 
            buffer[i] -= (buffer[i] >= L) * L; // modulo but faster
        }
    ...
    Vec<S, L> permute(
        ...
        if (shift > 0) {
            for (int i = 0; i < S; i++) {
                int index = i + shift;
                index -= (index >= S) * S; // modulo but faster
