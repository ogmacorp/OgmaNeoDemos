...
int main() {
    ...
    Array<Hierarchy::InputDesc> iDescs(1);
    iDescs[0].size = Int3(2, 2, sensorResolution);

    Array<Hierarchy::ActionDesc> aDescs(1);
    aDescs[0].size = Int3(1, 1, actionResolution);

    Array<Hierarchy::HiddenDesc> hDescs(3);

    for (int i = 0; i < hDescs.size(); i++)
        hDescs[i].size = Int3(3, 3, 16);

    Hierarchy h;
    h.initRandom(iDescs, aDescs, hDescs, 512);
    ...
    std::cout << "Sim test" << std::endl;

    for (int t2 = 0; t2 < 100000000; t2++) {
        env.step(0.0f);
    ...
    do {
        ...
        for (int it = 0; it < iters; it++) {
            ...
            h.step(inputCIs, reward, true);

            actionCIs[0] = h.getActionCIs(0)[0];
